...
const statusMessages = [
  ...
  let index = 0;
  const statusElement = document.querySelector(".loading-init__status");
  
  function updateStatus() {
    if (index < statusMessages.length) {
      statusElement.textContent = statusMessages[index];
      index++;
      setTimeout(updateStatus, 2000); // 2000ms mỗi bước
    } else {
      document.querySelector(".loading-init").classList.add("hide");
    }
  }
